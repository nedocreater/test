...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if service_name:
        ...
        keyboard = [
            [InlineKeyboardButton("🌐 Выбрать услугу на сайте", url="https://ваш-сайт.com")],
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🌐 Перейти на сайт", url="https://ваш-сайт.com")],
...
async def services_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    services_text = (
        ...
        "🌐 *Или выберите услугу на сайте:* https://ваш-сайт.com"
    )
...
async def website_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton("🌐 Открыть сайт", url="https://ваш-сайт.com")],
        [InlineKeyboardButton("🎨 Посмотреть услуги", callback_data="show_services")],
...
async def handle_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not thread:
            logger.error(f"Тред не найден для пользователя {user_id}")
            await message.reply_text(
                "⚠️ Произошла ошибка. Пожалуйста, попробуйте снова или свяжитесь с администратором."
            )
            return
        ...
        if message.text:
            try:
                ...
                if "Topic_deleted" in str(e):
                    # Тред был удален, создаем новый
                    logger.warning(f"Тред #{thread_id} был удален. Создаем новый...")
                    ...
                    # Создаем новый тред
                    new_topic, new_welcome_message = await create_forum_topic_for_user(
                        user_id, user_name, thread_id, selected_service or thread.get('selected_service'), context
                    )
                    
                    if not new_welcome_message:
                        await message.reply_text(
                            "⚠️ Не удалось создать новое обращение. Пожалуйста, свяжитесь с администратором."
                        )
                        return
                    
                    # Обновляем тред в БД с новыми данными
                    new_forum_topic_id = new_topic.message_thread_id if hasattr(new_topic, 'message_thread_id') else new_welcome_message.message_id
                    conn = Database.get_connection()
                    cursor = conn.cursor()
                    cursor.execute('''
                        UPDATE threads SET forum_topic_id = ?, forum_topic_message_id = ? WHERE thread_id = ?
                    ''', (new_forum_topic_id, new_welcome_message.message_id, thread_id))
                    conn.commit()
                    conn.close()
                    
                    # Отправляем сообщение в новый тред
                    group_reply = await context.bot.send_message(
                        chat_id=GROUP_CHAT_ID,
                    ...
                    await message.reply_text(
                        "⚠️ Ваше предыдущее обращение было пересоздано. Сообщение отправлено в новое обращение.",
                        reply_to_message_id=message.message_id
                    )
                else:
                    raise e
            ...
            try:
                ...
                if "Topic_deleted" in str(e):
                    ...
                    await message.reply_text(
                        "⚠️ Ваше предыдущее обращение было пересоздано. Фото отправлено в новое обращение.",
                        reply_to_message_id=message.message_id
                    )
                else:
                    raise e
            ...
            try:
                ...
                if "Topic_deleted" in str(e):
                    ...
                    await message.reply_text(
                        "⚠️ Ваше предыдущее обращение было пересоздано. Документ отправлен в новое обращение.",
                        reply_to_message_id=message.message_id
                    )
                else:
                    raise e
            ...
            try:
                ...
                if "Topic_deleted" in str(e):
                    ...
                    await message.reply_text(
                        "⚠️ Ваше предыдущее обращение было пересоздано. Видео отправлено в новое обращение.",
                        reply_to_message_id=message.message_id
                    )
                else:
                    raise e
            ...
            try:
                ...
                if "Topic_deleted" in str(e):
                    ...
                    await message.reply_text(
                        "⚠️ Ваше предыдущее обращение было пересоздано. Аудио отправлено в новое обращение.",
                        reply_to_message_id=message.message_id
                    )
                else:
                    raise e
            ...
            try:
                ...
                if "Topic_deleted" in str(e):
                    ...
                    group_reply = await context.bot.send_message(
                        chat_id=GROUP_CHAT_ID,
                        message_thread_id=new_forum_topic_id,
                        text=f"{message_prefix}📨 *Сообщение от клиента (тип: {message.content_type}, тред пересоздан)*",
                        parse_mode='Markdown'
                    )
